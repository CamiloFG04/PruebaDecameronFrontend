<template>
  ...
  <form class="login100-form validate-form p-b-33 p-t-5" @submit.prevent="onSubmit">
    ...
    <div class="wrap-input100 validate-input" data-validate="Enter username">
      <input class="input100" type="text" placeholder="Correo" required v-model="user.email" />
...
export default {
  setup(){
    ...
    return {
      ...
      onSubmit: async() => {
        try {
          const {success, message} = await loginUser(user.value)
          if(!success) Swal.fire('Error', message, 'error')
          else router.push({name:'hotel-list'})
